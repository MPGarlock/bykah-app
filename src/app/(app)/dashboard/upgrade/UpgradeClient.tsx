  ...
  return (
    <div style={{ backgroundColor: '#0A1628', minHeight: '100vh', padding: '3rem 1rem', fontFamily: 'DM Sans, sans-serif' }}>
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        <h1 style={{ color: '#C9973A', fontFamily: 'Cormorant Garamond, serif', fontSize: '2.5rem', textAlign: 'center', marginBottom: '0.5rem' }}>{isPro ? 'Your Membership' : 'Choose Your Plan'}</h1>
        <p style={{ color: '#CBD5E8', textAlign: 'center', marginBottom: '2rem' }}>{isPro ? 'Manage your BYKAH Pro membership' : 'Unlock the full BYKAH experience with Pro'}</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem' }}>
          <div style={{ backgroundColor: '#0f1e38', border: '1px solid #2a3a5c', borderRadius: '1rem', padding: '2rem' }}>
            ...
            <div style={{ marginTop: '1.5rem' }}>
              <Link href="/dashboard" style={{ display: 'block', textAlign: 'center', padding: '0.75rem', border: '1px solid #CBD5E8', borderRadius: '0.5rem', color: '#CBD5E8', textDecoration: 'none' }}>{isPro ? 'Included' : 'Current Plan'}</Link>
            </div>
          </div>
          <div style={{ backgroundColor: '#0f1e38', border: '2px solid #C9973A', borderRadius: '1rem', padding: '2rem', position: 'relative' }}>
            <div style={{ position: 'absolute', top: '-0.75rem', left: '50%', transform: 'translateX(-50%)', backgroundColor: '#C9973A', color: '#0A1628', padding: '0.25rem 1rem', borderRadius: '9999px', fontWeight: 700, fontSize: '0.75rem' }}>{isPro ? 'YOUR PLAN' : 'MOST POPULAR'}</div>
            ...
            {isPro ? (
              <>
                <div style={{ width: '100%', padding: '0.75rem', backgroundColor: 'transparent', color: '#C9973A', fontWeight: 700, borderRadius: '0.5rem', border: '1px solid #C9973A', textAlign: 'center', marginBottom: '0.75rem', fontSize: '1rem' }}>
                  Current Plan
                </div>
                <button
                  onClick={handleManageMembership}
                  disabled={portalLoading}
                  style={{ width: '100%', padding: '0.75rem', backgroundColor: portalLoading ? '#8a6b29' : '#C9973A', color: '#0A1628', fontWeight: 700, borderRadius: '0.5rem', border: 'none', cursor: portalLoading ? 'wait' : 'pointer', fontSize: '1rem' }}
                >
