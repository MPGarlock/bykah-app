...
export default function UpgradePage() {
  ...
  const price = billing === 'monthly' ? '$15' : '$144';
  const period = billing === 'monthly' ? '/mo' : '/yr';
  const savings = billing === 'annual' ? 'Save $36/yr' : '';
    ...
    <div style={{ backgroundColor: '#0A1628', minHeight: '100vh', padding: '3rem 1rem', fontFamily: 'DM Sans, sans-serif' }}>
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '0.5rem', marginBottom: '2.5rem' }}>
          ...
          <button onClick={() => setBilling('annual')} style={{ padding: '0.5rem 1.5rem', borderRadius: '9999px', border: '1px solid #C9973A', backgroundColor: billing === 'annual' ? '#C9973A' : 'transparent', color: billing === 'annual' ? '#0A1628' : '#C9973A', fontWeight: 600, cursor: 'pointer' }}>Annual - Save $36</button>
        </div>
        {error && (
